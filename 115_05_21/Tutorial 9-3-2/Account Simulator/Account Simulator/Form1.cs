...
namespace Account_Simulator
{
    public partial class Form1 : Form
    {
        ...
        private void depositButton_Click(object sender, EventArgs e)
        {
            ...
            {
                BankAccount account = searchAccount(depositAccounttextBox5.Text);
                if (account == null)
                {
                    return; // 如果帳戶不存在，則不進行存款操作
                }
        ...
        {                 
            ...
            {
                BankAccount account = searchAccount(withdrawAccountTextBox.Text); // 修正此行
                if (account == null)
                {
                    return; // 如果帳戶不存在，則不進行提款操作
                }
                // 只有提款成功時才更新顯示
        ...
        private TextBox withdrawAccountTextBox;
    }
}
